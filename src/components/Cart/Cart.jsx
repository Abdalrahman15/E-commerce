...
export default function Cart() {
  ...
  let{Cart,UpdateProduct,DeleteProduct,ClearCart}=useContext(CartContext)
...
<button  className=" m-3 text-lg font-medium  w-[1/7] h-[5%] top-[0px] left-1  text-white bg-main hover:bg-red-800 focus:ring-4 focus:outline-none focus:ring-blue-300 rounded-lg  px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">
  ...
  <table className="  w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
    ...
    <tbody>
     {Cart?.data?.data?.products?.map((product,index)=> <tr key={index} className="bg-white border-b dark:bg-gray-800 dark:border-gray-700 border-gray-200 hover:bg-gray-50 dark:hover:bg-gray-600">
        <td className="p-4">
        ...
        <td className="px-6 py-4 font-semibold text-gray-900 dark:text-white">
          {product?.price * product.count} <span>$</span>
        </td>
        <td className="px-6 py-4">
  ...
  <button onClick={()=>ClearCart()} className="w-full text-white bg-blue-800 hover:bg-red-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">
   Clear Cart

  </button>
  

</div>:<div className='flex justify-center items-center mt-[50px] p-52 bg-gray-300'>

  <h1 className='text-7xl'>Empty Cart</h1>
